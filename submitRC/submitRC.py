...
gettext.bindtextdomain("messages", os.path.join(sys.prefix, "submitRC/locale"))
...
def excepthook(type, value, tb):
    ...
    if type is Error and str(value):
        ...
        cprint(_("Sorry, something's wrong! Let fozatkardouh@gmail know!"), "yellow")
...
def submit(branch, silent=True):
    ...
    if not which("git"):
        raise Error(_("You don't have git. Install git, then re-run submitRC!"))
    ...
    if not matches or StrictVersion(matches.group(1)) < StrictVersion("2.7.0"):
        raise Error(
            _("You have an old version of git. Install version 2.7 or later, then re-run submitRC!"))
    ...
    url = "https://github.com/cs50/checks/raw/master/{}/submit50/exclude".format(branch)
    try:
    ...
    try:
        ...
        email = "{}@users.noreply.github.com".format(username)
        # change repo -= 2 in the end
        repo = "git@github.com:RefugeesCodeAT/cs50-psets-{}.git".format(username)
        progress(False)
    ...
    # authenticate user via HTTPS
        ...
        repo = "https://{}@github.com/RefugeesCodeAT/cs50-psets-{}".format(username, username)
    ...
    try:
        ...
        if password:
            e = Error(_("Looks like submitRC isn't enabled for your account yet. "
                        "Go to https://cs50.me/authorize and make sure you accept any pending invitations!"))
        else:
            e = Error(
                _(
                    "Looks like you have the wrong username in ~/.gitconfig or submitRC isn't yet enabled for "
                    "your account. "
                    "Double-check ~/.gitconfig and then log into https://cs50.me/ in a browser, "
                    "click \"Authorize application\" if prompted, and re-run submitRC here."))
        e.__cause__ = None
        raise e
    ...
    if len(huges) > 0:
        raise Error(_("These files are too large to be submitted:\n{}\n"
                      ...
                      "and then re-run submitRC!").format("\n".join(huges)))
    elif len(larges) > 0:
    ...
    if not silent:
        cprint(_("Uploaded to {}!").format(branch), "green")
    progress(False)
...
def two_factor(username, password):
    ...
    while True:
        ...
        if code:
            break
    data = json.dumps({"scopes": ["repo", "user"], "note": "submitRC {}".format(timestamp)})
