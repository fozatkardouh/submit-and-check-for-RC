...
def main():
    ...
    if not config.args.local:
        try:
            ...
            import submit50 as submit50
            # Submit to checkRC repo.
            import submitRC as submitRC
        except ImportError:
            raise InternalError(
                "submit50 and/or submitRC not installed. Install both and run check50 again.")
        else:
            ...
            submitRC.handler.type = "check"
            signal.signal(signal.SIGINT, submitRC.handler)
            submitRC.run.verbose = config.args.verbose

            submitRC.submit(identifier)
...
class App(object):
    ...
    def raw_content(self, output=None, str_output=None):
        ...
        return self._search_page(output, str_output, self.response.data,
                                 lambda regex, content: regex.search(content.decode()))
